...
unsafe extern "C" fn mygetpixel(
    ...
    unsafe {
        ...
        let mut start: std::ffi::c_int =
            (pos * width + 4 as std::ffi::c_int) / 8 as std::ffi::c_int;
...
unsafe extern "C" fn X_AllocColors(d: *mut xdriverdata) {
    unsafe {
        ...
        c.red = (65535 as std::ffi::c_ulong).wrapping_div(2 as std::ffi::c_int as std::ffi::c_ulong)
            as std::ffi::c_ushort;
        c.green = (65535 as std::ffi::c_ulong)
            .wrapping_div(2 as std::ffi::c_int as std::ffi::c_ulong)
            as std::ffi::c_ushort;
        c.blue = 65535 as std::ffi::c_ulong as std::ffi::c_ushort;
        if XAllocColor((*d).dp, (*d).cmap, &mut c) == 0 {
...
unsafe extern "C" fn X_setinversionmode(inverted: std::ffi::c_int, d: *mut xdriverdata) {
    unsafe {
        (*d).inverted = inverted;
        ...
        *driverdata = d as *mut std::ffi::c_void;
        ...
        (*d).wi = XCreateWindow(
            ...
            ((1 as std::ffi::c_long) << 1 as std::ffi::c_int
                ...
                | (1 as std::ffi::c_long) << 11 as std::ffi::c_int)
                as std::ffi::c_ulong,
            &mut (*d).attr,
        );
        if registered == 0 {
            ...
            if (*d).pi != 0 {
                ...
                if !image.is_null() {
                        ...
                        while y < (*d).fontheight {
                            ...
                            o = (((mygetpixel(image, 0 as std::ffi::c_int, i * (*d).fontheight + y)
                                ...
                                + (((mygetpixel(
                                    ...
                                    as std::ffi::c_int)
                                    << 0 as std::ffi::c_int);
                            *data.offset((i * (*d).fontheight + y) as isize) =
                                o as std::ffi::c_uchar;
                            y += 1;
                            y;
                        }
                        i += 1;
                    ...
                    aafont.name =
                        b"Font used by X server\0" as *const u8 as *const std::ffi::c_char;
        ...
        if tmp != 0 {
            ...
            if (getenv(b"AABlink\0" as *const u8 as *const std::ffi::c_char)).is_null() {
                (*d).pi = XCreatePixmap(
                    ...
                    (*d).pixelwidth as std::ffi::c_uint,
                    (*d).pixelheight as std::ffi::c_uint,
                    (*((*((*d).dp as _XPrivDisplay)).screens).offset((*d).screen as isize))
                        .root_depth as std::ffi::c_uint,
                );
            } else {
...
unsafe fn X_getsize(c: *mut aa_context, width1: &mut i64, height1: &mut i64) {
    unsafe {
        let d: *mut xdriverdata = (*c).driverdata as *mut xdriverdata;
        __aa_X_getsize(c, d);
...
unsafe fn X_setattr(d: *mut xdriverdata, attr: i64) {
    unsafe {
        match attr {
            0 | 4 => {
                (*d).currGC = (*d).normalGC;
...
unsafe extern "C" fn alloctables(d: *mut xdriverdata) {
    unsafe {
        _texty = malloc(
...
unsafe fn X_flush(c: *mut aa_context) {
    unsafe {
        let d: *mut xdriverdata = (*c).driverdata as *mut xdriverdata;
        let mut x = 0;
        let mut y = 0;
                    ...
                    if (*nitem.offset(y as isize))[x as usize] != 0 {
                        ...
                        if x == 4 as std::ffi::c_int {
                            XDrawText(
                                ...
                                (*d).currGC,
                                1 as std::ffi::c_int,
                                (y + 1 as std::ffi::c_int) * (*d).fontheight
                                    - (*(*d).font_s).descent,
                                &mut *_texty.offset(((y * 5 + x) * (*d).width + 0) as isize),
                                (*nitem.offset(y as isize))[x as usize],
                            );
                    ...
                    && nrectangles[3 as std::ffi::c_int as usize] < 5 as std::ffi::c_int
                {
                    ...
                    while i < nrectangles[3 as std::ffi::c_int as usize] {
                        XClearArea(
                            ...
                            (*d).wi,
                            (*_rectangles.offset(
                                (3 as std::ffi::c_int * (*d).height * (*d).width + i) as isize,
                            ))
                            .x as std::ffi::c_int,
                            (*_rectangles.offset(
                                (3 as std::ffi::c_int * (*d).height * (*d).width + i) as isize,
...
pub unsafe fn __aa_X_redraw(c: *mut aa_context) {
    unsafe {
        let d: *mut xdriverdata = (*c).driverdata as *mut xdriverdata;
...
unsafe fn X_gotoxy(c: *mut aa_context, x: i64, y: i64) {
    unsafe {
...
unsafe fn X_cursor(c: *mut aa_context, mode: i64) {
    unsafe {
        let d: *mut xdriverdata = (*c).driverdata as *mut xdriverdata;
