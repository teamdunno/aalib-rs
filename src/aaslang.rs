...
unsafe fn slang_getsize(c: *mut aa_context, width: &mut i64, height: &mut i64) {
    unsafe {
...
unsafe fn slang_print(c: *mut aa_context, text: *const std::ffi::c_char) {
    unsafe {
        SLsmg_write_string(text as *mut std::ffi::c_char);
    }
}
unsafe fn slang_flush(c: *mut aa_context) {
    unsafe {
        SLsmg_refresh();
    }
}
unsafe fn slang_gotoxy(c: *mut aa_context, x: i64, y: i64) {
    unsafe {
        SLsmg_gotorc(y as std::ffi::c_int, x as std::ffi::c_int);
    }
}
unsafe fn slang_cursor(c: *mut aa_context, mode: i64) {
    unsafe {
