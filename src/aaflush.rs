...
unsafe fn aa_display(mut c: *mut aa_context, mut x1: i64, mut y1: i64, mut x2: i64, mut y2: i64) {
    let mut x = 0;
    let mut y = 0;
    let mut pos = 0;
    let mut attr = 0;
...
pub fn aa_hidemouse(mut c: *mut aa_context) {
    unsafe {
        if (*c).mousemode != 0 {
            (*c).mousemode = 0;
...
pub fn aa_showmouse(mut c: *mut aa_context) {
    unsafe {
        if (*c).mousemode == 0 {
            (*c).mousemode = 1;
            if !((*c).mousedriver).is_null() && ((*(*c).mousedriver).cursormode).is_some() {
...
pub fn aa_flush(mut c: *mut aa_context) {
    unsafe {
